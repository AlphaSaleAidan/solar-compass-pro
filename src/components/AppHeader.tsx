...
import { Zap, LogOut, User, Crown, ChevronDown, Settings } from 'lucide-react';
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-[58px] flex items-center px-6 gap-2 border-b border-border backdrop-blur-xl" style={{ background: isPlus ? 'rgba(255,255,255,0.97)' : 'rgba(7,9,13,0.97)' }}>
        ...
        <div className="flex items-center gap-3 ml-auto">
          ...
          <div className="relative" ref={menuRef}>
            <button onClick={() => switchableRoles.length > 1 && setShowRoleMenu(!showRoleMenu)}
          ...
          <button onClick={() => setShowSettings(true)}
            className={`w-8 h-8 rounded-full flex items-center justify-center text-sm border cursor-pointer hover:border-primary transition-colors ${isPlus ? 'bg-gray-100 border-gray-200' : 'bg-bg4 border-border2'}`}>
          ...
          <button onClick={() => setShowSettings(true)}
            className={`p-1.5 rounded-md border transition-all ${
              isPlus ? 'bg-gray-50 border-gray-200 text-gray-500 hover:border-primary hover:text-primary' : 'bg-bg3 border-border text-muted-foreground hover:border-primary hover:text-primary'
            }`}>
